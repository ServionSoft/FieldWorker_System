...
import { parseInvoiceSettings } from '../modules/company/invoice-settings.js';
...
async function logoPathFor(client, companyId, fileId, showLogo) {
    if (!showLogo || !fileId)
        return null;
    const { rows } = await client.query(`SELECT storage_key, mime_type FROM files WHERE company_id = $1 AND id = $2`, [companyId, fileId]);
    const file = rows[0];
    if (!file)
        return null;
    if (!/^image\/(jpeg|jpg|png)$/i.test(String(file.mime_type || '')))
        return null;
...
function drawCompanyHeader(doc, opts) {
    ...
    if (opts.settings.showAddress && opts.address)
        doc.text(String(opts.address), textX, doc.y, { width: 385 });
    const bits = [
    ...
    if (bits.length)
        doc.text(bits.join('  ·  '), textX, doc.y, { width: 385 });
...
function drawParty(doc, heading, lines) {
    if (!lines.length)
        return;
...
function drawTotals(doc, y, settings, subtotal, tax, total) {
    const totalsX = 390;
    const colsTotal = 470;
    doc.font('Helvetica').fontSize(10);
...
function drawBankDetails(doc, settings, y) {
    ...
    if (!lines.length && !settings.paymentInstructions)
        return y;
    y += 20;
    if (y > 680) {
        doc.addPage();
...
function drawFooter(doc, settings, footer) {
    if (!settings.showFooter || !footer)
        return;
    doc.font('Helvetica').fontSize(8).fillColor('#555555');
    doc.text(String(footer), 50, 720, { width: 510, align: 'center' });
}
export async function buildInvoicePdf(client, companyId, invoiceId) {
    const inv = await client.query(`SELECT i.*,
     ...
     WHERE i.company_id = $1 AND i.id = $2`, [companyId, invoiceId]);
    const i = inv.rows[0];
    if (!i)
        return null;
    ...
    const items = (await client.query(`SELECT description, quantity, unit_price, total FROM invoice_line_items WHERE invoice_id = $1 ORDER BY id`, [invoiceId])).rows;
    const addr = await client.query(`SELECT street, unit, city, state, zip
     FROM addresses
     WHERE company_id = $1 AND customer_id = $2
     ORDER BY is_default DESC, location_name
     LIMIT 1`, [companyId, i.customer_id]);
    const a = addr.rows[0];
    const billTo = [
...
export async function buildEstimatePdf(client, companyId, estimateId) {
    const est = await client.query(`SELECT e.*,
       c.name AS company_name, c.email AS company_email, c.phone AS company_phone,
       c.address AS company_address, c.website, c.invoice_footer, c.logo_file_id, c.invoice_settings,
       cc.first_name, cc.last_name, cc.email AS customer_email, cc.phone AS customer_phone,
     ...
     WHERE e.company_id = $1 AND e.id = $2`, [companyId, estimateId]);
    const e = est.rows[0];
    if (!e)
        return null;
    ...
    const items = (await client.query(`SELECT description, quantity, unit_price, total FROM estimate_line_items WHERE estimate_id = $1 ORDER BY id`, [estimateId])).rows;
    const billTo = [
    ...
    if (settings.showDueDate && e.valid_until)
        doc.text(`Valid until: ${fmtDate(e.valid_until)}`);
    if (settings.showStatus)
