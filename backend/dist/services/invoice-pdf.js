...
export async function buildInvoicePdf(client, companyId, invoiceId) {
    ...
    if (i.company_address)
        doc.text(String(i.company_address));
    ...
    for (const line of billTo)
        doc.text(line);
    if (i.customer_email)
        doc.text(String(i.customer_email));
    if (i.customer_phone)
        doc.text(String(i.customer_phone));
    doc.moveDown(1);
...
export async function buildEstimatePdf(client, companyId, estimateId) {
    ...
    if (e.company_address)
        doc.text(String(e.company_address));
    ...
    for (const line of billTo)
        doc.text(line);
    if (e.customer_email)
        doc.text(String(e.customer_email));
    if (e.customer_phone)
        doc.text(String(e.customer_phone));
    if (e.description) {
        doc.moveDown(0.6);
