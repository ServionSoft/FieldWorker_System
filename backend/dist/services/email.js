...
async function resolveSmtpIpv4(host) {
    ...
    try {
        ...
        if (addresses[0])
            return { host: addresses[0], servername: trimmed };
    }
    catch {
        /* try lookup next */
    }
...
async function smtpTransportOptions(cfg) {
...
async function transporter(cfg) {
...
function smtpErrorMessage(err) {
    ...
    if (e.code && !detail.includes(e.code))
...
function skipLog(channel, to, subject, reason, text) {
...
export async function loadPlatformSmtp() {
    const { rows } = await pool.query(`SELECT * FROM platform_settings WHERE id = 1`);
    const s = rows[0];
    if (!s?.smtp_host || !s?.smtp_from_email)
        return null;
...
export async function loadTenantSmtp(client, companyId) {
    const { rows } = await client.query(`SELECT smtp_host, smtp_port, smtp_user, smtp_password_enc, smtp_secure, smtp_from_name, smtp_from_email, smtp_reply_to
...
function expandLinkAliases(vars) {
...
function actionFromVars(vars, templateType) {
...
function platformEmailHtml(text, action) {
...
export async function loadPlatformTemplate(type) {
    const { rows } = await pool.query(`SELECT name, subject, body FROM platform_email_templates WHERE type = $1`, [type]);
    return rows[0];
}
export async function loadTenantTemplate(client, companyId, type) {
    const { rows } = await client.query(`SELECT id, name, subject, body, type FROM email_templates
     WHERE company_id = $1 AND type = $2
     ORDER BY name ASC LIMIT 1`, [companyId, type]);
    return rows[0];
}
...
export async function sendPlatformMail(to, subject, text) {
    return sendPlatformEmail({ to, subject, text });
}
export async function sendPlatformSmtpTest(to) {
    try {
        const cfg = await loadPlatformSmtp();
        if (!cfg)
        ...
        if (result.ok)
            return { ok: true, evidence: result.receipt };
        return { ok: false, error: result.error, evidence: result.receipt };
    }
    catch (err) {
        return { ok: false, error: err instanceof Error ? err.message : 'Platform SMTP test failed' };
    }
}
/** Loads tenant SMTP in a short DB transaction, then sends outside the transaction. */
export async function sendTenantSmtpTest(companyId, to) {
    try {
        ...
        return { ok: false, error: err instanceof Error ? err.message : 'Company SMTP test failed' };
    }
}
