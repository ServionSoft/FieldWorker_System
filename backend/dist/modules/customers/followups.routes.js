...
async function assertCompanyUser(client, companyId, userId) {
    const { rowCount } = await client.query(`SELECT 1 FROM company_members WHERE company_id = $1 AND user_id = $2 AND status = 'active'`, [companyId, userId]);
    return (rowCount ?? 0) > 0;
}
followUpsRouter.get('/', tenantRoute(async (req, res, client) => {
    ...
    const { rows } = await client.query(`SELECT f.*, coalesce(cc.first_name || ' ' || cc.last_name, 'Customer') AS customer_name
     FROM follow_ups f
     LEFT JOIN LATERAL (
     ...
     ORDER BY f.done ASC, f.due_date NULLS LAST, f.created_at DESC`, [companyId, customerId, done, overdue, dueToday, upcoming, mine, req.auth.userId]);
...
followUpsRouter.post('/', requirePermission('customers.write'), tenantRoute(async (req, res, client) => {
    ...
    const created = await client.query(`INSERT INTO follow_ups (company_id, customer_id, title, due_date, estimate_id, job_id, assigned_user_id)
     VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`, [req.auth.companyId, body.customerId, body.title, body.dueDate || null, body.estimateId ?? null, body.jobId ?? null, assigned]);
    await audit(client, {
...
followUpsRouter.patch('/:id', requirePermission('customers.write'), tenantRoute(async (req, res, client) => {
    ...
    let assignedId = body.assignedUserId;
    if (assignedId && !(await assertCompanyUser(client, req.auth.companyId, assignedId))) {
        assignedId = undefined;
    }
    const existing = await client.query(`SELECT * FROM follow_ups WHERE company_id = $1 AND id = $2`, [req.auth.companyId, req.params.id]);
    if (!existing.rowCount)
        throw notFound('Follow-up');
    const r = await client.query(`UPDATE follow_ups SET
...
followUpsRouter.delete('/:id', requirePermission('customers.write'), tenantRoute(async (req, res, client) => {
    const r = await client.query(`DELETE FROM follow_ups WHERE company_id = $1 AND id = $2 RETURNING id`, [req.auth.companyId, req.params.id]);
    if (!r.rowCount)
        throw notFound('Follow-up');
    res.json({ ok: true });
}));
