...
async function applySubscription(sub) {
    ...
    await pool.query(`UPDATE companies SET
       ...
       status = $5
     WHERE id = $1`, [company.id, sub.id, typeof sub.customer === 'string' ? sub.customer : null, planId, status]);
}
    ...
    try {
        if (event.type === 'checkout.session.completed') {
            ...
            if (companyId && session.subscription) {
                ...
                await pool.query(`UPDATE companies SET stripe_subscription_id = $2, stripe_customer_id = coalesce(stripe_customer_id, $3), status = 'active', plan_id = coalesce($4::uuid, plan_id)
           WHERE id = $1`, [companyId, subId, typeof session.customer === 'string' ? session.customer : null, session.metadata?.planId]);
            ...
            const company = await companyByStripe(typeof inv.customer === 'string' ? inv.customer : inv.customer?.id, subId, inv.metadata?.companyId);
            if (company) {
