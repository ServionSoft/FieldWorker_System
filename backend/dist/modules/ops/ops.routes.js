...
opsRouter.get('/calendar', requireFeature('calendar'), tenantRoute(async (req, res, client) => {
    ...
    const jobs = await client.query(`SELECT j.id, j.title, j.status, j.scheduled_date, j.scheduled_time, j.estimated_duration_hours, j.category,
     ...
     GROUP BY j.id, cc.first_name, cc.last_name`, [companyId, from, to]);
    const estimates = await client.query(`SELECT e.id, e.estimate_number, e.status, e.requested_on, e.category, cc.first_name, cc.last_name
     FROM estimates e
     ...
     WHERE e.company_id = $1 AND e.status <> 'converted' AND coalesce(e.requested_on, e.created_at::date) BETWEEN $2 AND $3`, [companyId, from, to]);
    res.json({
...
opsRouter.get('/dispatch', requireFeature('dispatch'), tenantRoute(async (req, res, client) => {
    ...
    const from = String(req.query.from ?? '2000-01-01');
    const to = String(req.query.to ?? '2100-01-01');
    const workers = await client.query(`SELECT m.user_id, u.name FROM worker_profiles wp
     JOIN company_members m ON m.id = wp.member_id
     JOIN users u ON u.id = m.user_id
     WHERE wp.company_id = $1 AND wp.employment_status = 'active'`, [companyId]);
    const jobs = await client.query(`SELECT j.id, j.title, j.status, j.scheduled_date, j.scheduled_time, j.estimated_duration_hours, j.category,
     ...
     LEFT JOIN job_assignees ja ON ja.job_id = j.id
     LEFT JOIN company_members m ON m.id = ja.member_id
     WHERE j.company_id = $1 AND j.scheduled_date BETWEEN $2 AND $3`, [companyId, from, to]);
    res.json({
...
opsRouter.get('/dashboard', tenantRoute(async (req, res, client) => {
    ...
    const workers = await client.query(`SELECT count(*) FILTER (WHERE employment_status = 'active')::int AS active FROM worker_profiles WHERE company_id = $1`, [companyId]);
    const inv = await client.query(`SELECT count(*) FILTER (WHERE quantity < min_stock)::int AS low FROM inventory_items WHERE company_id = $1`, [companyId]);
    const revenue = await client.query(`SELECT coalesce(sum(total),0) AS paid FROM invoices WHERE company_id = $1 AND status = 'paid'`, [companyId]);
    const byStatus = (s) => jobs.rows.filter((j) => j.status === s).length;
    const today = new Date().toISOString().slice(0, 10);
    const overdueFu = await client.query(`SELECT f.id, f.title, f.due_date, f.customer_id,
     ...
     ORDER BY f.due_date ASC LIMIT 8`, [companyId]);
    const dueTodayFu = await client.query(`SELECT f.id, f.title, f.due_date, f.customer_id,
            coalesce(cc.first_name || ' ' || cc.last_name, 'Customer') AS customer_name
     FROM follow_ups f
     LEFT JOIN LATERAL (
       SELECT first_name, last_name FROM customer_contacts WHERE customer_id = f.customer_id ORDER BY is_primary DESC LIMIT 1
     ...
     ORDER BY f.title ASC LIMIT 8`, [companyId]);
    const unpaid = await client.query(`SELECT i.id, i.invoice_number, i.total, i.due_date, i.status,
            coalesce(cc.first_name || ' ' || cc.last_name, 'Customer') AS customer_name
     FROM invoices i
     LEFT JOIN LATERAL (
     ...
     ORDER BY i.due_date ASC LIMIT 8`, [companyId]);
    const leads = await client.query(`SELECT count(*)::int AS n FROM customers WHERE company_id = $1 AND status = 'lead'`, [companyId]);
    res.json({
...
opsRouter.get('/reports', requireFeature('reports'), tenantRoute(async (req, res, client) => {
    ...
    const workers = await client.query(`SELECT u.name, wp.jobs_completed, wp.rating
...
searchRouter.use(requireAuth, requireTenant, requireRole('admin', 'field_worker'), requirePermission('customers.read', 'jobs.read', 'estimates.read', 'invoices.read', 'workers.manage', 'documents.access', 'communications.access'));
searchRouter.get('/search', tenantRoute(async (req, res, client) => {
    ...
    const empty = { rows: [] };
    const customers = have.has('customers.read') ? await client.query(`SELECT c.id,
     ...
     LIMIT 8`, [companyId, like, phone]) : empty;
    const jobs = have.has('jobs.read') ? await client.query(`SELECT j.id, j.title, coalesce(cc.first_name || ' ' || cc.last_name, '') AS customer_name
     FROM jobs j
     LEFT JOIN LATERAL (
     ...
     LIMIT 8`, [companyId, like, req.auth.role, req.auth.memberId]) : empty;
    const estimates = have.has('estimates.read') ? await client.query(`SELECT e.id, e.estimate_number, coalesce(cc.first_name || ' ' || cc.last_name, '') AS customer_name
     FROM estimates e
     LEFT JOIN LATERAL (
       SELECT first_name, last_name FROM customer_contacts
     ...
     LIMIT 8`, [companyId, like]) : empty;
    const invoices = have.has('invoices.read') ? await client.query(`SELECT i.id, i.invoice_number, coalesce(cc.first_name || ' ' || cc.last_name, '') AS customer_name
     FROM invoices i
     LEFT JOIN LATERAL (
       SELECT first_name, last_name FROM customer_contacts
    ...
    const workers = have.has('workers.manage') || have.has('jobs.read')
        ? await client.query(`SELECT u.id, u.name, u.email, u.phone
       ...
       ORDER BY u.name
       LIMIT 8`, [companyId, like])
        : { rows: [] };
    const documents = have.has('documents.access')
        ? await client.query(`SELECT d.id, f.original_name, d.job_id
    ...
    const communications = have.has('communications.access')
        ? await client.query(`SELECT id, type, direction, coalesce(body, '') AS body, customer_id, job_id
       FROM communications
