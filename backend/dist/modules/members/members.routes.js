...
async function mapMember(client, companyId, memberId) {
    const { rows } = await client.query(`SELECT m.id, m.role, m.status, m.created_at, m.notify_email_assignments, m.notify_email_invoices, m.notify_email_billing,
    ...
    if (!m)
        return null;
    const overrides = (await client.query(`SELECT permission, allowed FROM company_member_permissions WHERE member_id = $1`, [memberId])).rows;
    const permissions = effectivePermissions(m.role, overrides);
    return {
        id: m.id,
        userId: m.user_id,
        name: m.name,
...
membersRouter.get('/', tenantRoute(async (req, res, client) => {
    const { rows } = await client.query(`SELECT id FROM company_members WHERE company_id = $1 ORDER BY created_at`, [req.auth.companyId]);
    const items = [];
    for (const r of rows)
        items.push(await mapMember(client, req.auth.companyId, r.id));
...
membersRouter.get('/invitations', tenantRoute(async (req, res, client) => {
    const { rows } = await client.query(`SELECT id, email, name, role, expires_at, accepted_at, created_at
     FROM invitations WHERE company_id = $1 ORDER BY created_at DESC`, [req.auth.companyId]);
    res.json({
        items: rows.map((r) => ({
            id: r.id,
            email: r.email,
...
membersRouter.post('/invite', tenantRoute(async (req, res, client) => {
    ...
    const created = await client.query(`INSERT INTO invitations (company_id, email, name, role, token_hash, invited_by, expires_at)
     VALUES ($1,$2,$3,$4,$5,$6, now() + interval '7 days') RETURNING *`, [companyId, body.email, body.name ?? null, body.role, sha256(token), req.auth.userId]);
...
membersRouter.patch('/:id', tenantRoute(async (req, res, client) => {
    ...
    if (current.role === 'owner' && body.role && body.role !== 'owner') {
        const owners = await client.query(`SELECT count(*)::int AS n FROM company_members WHERE company_id = $1 AND role = 'owner' AND status = 'active'`, [companyId]);
    ...
    if (current.role === 'owner' && body.status === 'inactive') {
        const owners = await client.query(`SELECT count(*)::int AS n FROM company_members WHERE company_id = $1 AND role = 'owner' AND status = 'active'`, [companyId]);
        if (owners.rows[0].n <= 1)
            throw badRequest('Cannot deactivate the last owner');
    }
    if (body.role === 'owner' && req.auth.role !== 'owner')
    ...
    if (nextRole === 'field_worker') {
        await client.query(`INSERT INTO worker_profiles (company_id, member_id, employment_status)
       VALUES ($1,$2,'active')
       ON CONFLICT (member_id) DO UPDATE SET
         employment_status = CASE WHEN $3 = 'active' THEN 'active' ELSE worker_profiles.employment_status END`, [companyId, req.params.id, nextStatus]);
    }
    if (body.permissions) {
        ...
        for (const p of body.permissions) {
            ...
            if (p.allowed === isDefault)
                continue;
            await client.query(`INSERT INTO company_member_permissions (company_id, member_id, permission, allowed) VALUES ($1,$2,$3,$4)`, [companyId, req.params.id, p.permission, p.allowed]);
        }
    }
    await audit(client, {
...
membersRouter.delete('/:id', tenantRoute(async (req, res, client) => {
    ...
    if (current.role === 'owner') {
        const owners = await client.query(`SELECT count(*)::int AS n FROM company_members WHERE company_id = $1 AND role = 'owner' AND status = 'active'`, [companyId]);
        if (owners.rows[0].n <= 1)
            throw badRequest('Cannot remove the last owner');
    }
...
invitePublicRouter.get('/:token', wrap(async (req, res) => {
    const preview = await withTransaction(async (client) => {
        ...
        const { rows } = await client.query(`SELECT i.email, i.name, i.role, i.expires_at, i.accepted_at, c.name AS company_name
       FROM invitations i JOIN companies c ON c.id = i.company_id
       WHERE i.token_hash = $1`, [sha256(req.params.token)]);
...
invitePublicRouter.post('/:token/accept', wrap(async (req, res) => {
    ...
    const accepted = await withTransaction(async (client) => {
        await client.query(`SELECT set_config('app.is_super_admin', 'true', true)`);
        const { rows } = await client.query(`SELECT * FROM invitations WHERE token_hash = $1 FOR UPDATE`, [sha256(req.params.token)]);
        const inv = rows[0];
        ...
        if (existing.rowCount) {
            ...
            const created = await client.query(`INSERT INTO users (email, password_hash, name) VALUES ($1,$2,$3) RETURNING id`, [inv.email, await hashPassword(body.password), body.name]);
            userId = created.rows[0].id;
        }
        const dup = await client.query(`SELECT id FROM company_members WHERE company_id = $1 AND user_id = $2`, [inv.company_id, userId]);
        if (dup.rowCount)
        ...
        if (inv.role === 'field_worker') {
            await client.query(`INSERT INTO worker_profiles (company_id, member_id, employment_status) VALUES ($1,$2,'active')`, [inv.company_id, member.rows[0].id]);
        }
