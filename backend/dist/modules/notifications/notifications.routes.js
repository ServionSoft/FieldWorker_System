...
notificationsRouter.get('/', tenantRoute(async (req, res, client) => {
    const { rows } = await client.query(`SELECT * FROM notifications WHERE user_id = $1 ORDER BY created_at DESC`, [req.auth.userId]);
...
notificationsRouter.post('/:id/read', tenantRoute(async (req, res, client) => {
    const r = await client.query(`UPDATE notifications SET read = true WHERE id = $1 AND user_id = $2 RETURNING id`, [req.params.id, req.auth.userId]);
    if (!r.rowCount)
