...
async function map(client, companyId, id) {
    const { rows } = await client.query(`SELECT sa.*, cc.first_name, cc.last_name
     ...
     WHERE sa.company_id = $1 AND sa.id = $2`, [companyId, id]);
    const a = rows[0];
    if (!a)
        return null;
    return {
...
agreementsRouter.get('/', tenantRoute(async (req, res, client) => {
    ...
    const items = [];
    for (const r of rows)
        items.push(await map(client, req.auth.companyId, r.id));
...
agreementsRouter.post('/', tenantRoute(async (req, res, client) => {
    ...
    const created = await client.query(`INSERT INTO service_agreements (company_id, customer_id, job_id, title, terms, status, start_date, end_date)
     VALUES ($1,$2,$3,$4,$5,'draft',$6,$7) RETURNING id`, [req.auth.companyId, body.customerId, body.jobId ?? null, body.title, body.terms, body.startDate || null, body.endDate || null]);
...
agreementsRouter.patch('/:id', tenantRoute(async (req, res, client) => {
    ...
    const r = await client.query(`UPDATE service_agreements SET
       status = coalesce($3, status), title = coalesce($4, title), terms = coalesce($5, terms)
     WHERE company_id = $1 AND id = $2 RETURNING id`, [req.auth.companyId, req.params.id, body.status ?? null, body.title ?? null, body.terms ?? null]);
    if (!r.rowCount)
        throw notFound('Agreement');
