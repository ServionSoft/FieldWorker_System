...
async function loadWorkerSnapshot(companyId, memberId) {
    ...
    try {
        ...
        const { rows } = await client.query(`SELECT wp.id, wp.employment_status, wp.rating, wp.jobs_completed
       FROM worker_profiles wp WHERE wp.member_id = $1 AND wp.company_id = $2`, [memberId, companyId]);
        const wp = rows[0];
        if (!wp)
            return null;
        const specs = (await client.query(`SELECT name FROM worker_specialties WHERE worker_profile_id = $1 ORDER BY name`, [wp.id])).rows.map((r) => r.name);
        const availRows = (await client.query(`SELECT weekday, start_time, end_time FROM worker_availability WHERE worker_profile_id = $1`, [wp.id])).rows;
        const availability = {};
        DAYS.forEach((d, i) => {
        ...
        return {
            specialties: specs,
            employmentStatus: wp.employment_status,
...
async function buildProfile(auth) {
    const userId = auth.userId;
    const { rows: users } = await pool.query(`SELECT id, email, name, phone, avatar_key, is_platform_admin,
    ...
    if (user.is_platform_admin || auth.role === 'super_admin') {
        ...
        const { rows: members } = await pool.query(`SELECT m.company_id, m.role, m.status, m.notify_email_assignments, m.notify_email_invoices, m.notify_email_billing,
              c.name AS company_name
       FROM company_members m
       JOIN companies c ON c.id = m.company_id
       WHERE m.user_id = $1 AND m.id = $2 AND c.deleted_at IS NULL`, [userId, auth.memberId]);
...
profileRouter.get('/', wrap(async (req, res) => {
    res.json(await buildProfile(req.auth));
}));
profileRouter.patch('/', wrap(async (req, res) => {
    const auth = req.auth;
    const userId = auth.userId;
    if (auth.impersonatedBy)
        throw forbidden('Cannot edit a profile while impersonating');
    const body = z.object({
    ...
    await withTransaction(async (client) => {
        const current = (await client.query(`SELECT name, first_name, last_name FROM users WHERE id = $1`, [userId])).rows[0];
...
profileRouter.post('/change-password', wrap(async (req, res) => {
    const auth = req.auth;
    if (auth.impersonatedBy)
        throw forbidden('Cannot change a password while impersonating');
    const body = z.object({
    ...
    await withTransaction(async (client) => {
       ...
       VALUES ($1,$2, now() + interval '14 days')`, [auth.userId, sha256(refreshToken)]);
        await audit(client, {
...
profileRouter.post('/avatar', avatarUpload.single('file'), wrap(async (req, res) => {
    const auth = req.auth;
    if (auth.impersonatedBy)
        throw forbidden('Cannot change an avatar while impersonating');
    if (!req.file)
    ...
    await withTransaction(async (client) => {
        ...
        await client.query(`UPDATE users SET avatar_key = $2, avatar_url = $3 WHERE id = $1`, [auth.userId, storageKey, '/api/profile/avatar']);
