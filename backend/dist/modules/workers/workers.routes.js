...
async function mapWorker(client, companyId, profileId) {
    const { rows } = await client.query(`SELECT wp.*, m.user_id, m.status AS member_status, u.name, u.email, u.phone, u.avatar_url
     FROM worker_profiles wp
     JOIN company_members m ON m.id = wp.member_id
     JOIN users u ON u.id = m.user_id
     WHERE wp.company_id = $1 AND wp.id = $2`, [companyId, profileId]);
    const w = rows[0];
    if (!w)
        return null;
    const specs = (await client.query(`SELECT name FROM worker_specialties WHERE worker_profile_id = $1`, [profileId])).rows.map((r) => r.name);
    const avail = (await client.query(`SELECT weekday, start_time, end_time FROM worker_availability WHERE worker_profile_id = $1`, [profileId])).rows;
    const off = (await client.query(`SELECT off_date FROM worker_time_off WHERE worker_profile_id = $1 ORDER BY off_date`, [profileId])).rows.map((r) => r.off_date.toISOString().slice(0, 10));
    return {
        id: w.user_id,
        profileId: w.id,
...
async function findProfileIdByUser(client, companyId, userId) {
    const { rows } = await client.query(`SELECT wp.id FROM worker_profiles wp
     JOIN company_members m ON m.id = wp.member_id
     WHERE wp.company_id = $1 AND m.user_id = $2`, [companyId, userId]);
    return rows[0]?.id;
}
workersRouter.get('/', requireRole('admin', 'field_worker'), tenantRoute(async (req, res, client) => {
    ...
    const items = [];
    for (const r of rows)
        items.push(await mapWorker(client, companyId, r.id));
...
workersRouter.post('/', requireRole('admin'), requirePermission('workers.manage'), tenantRoute(async (req, res, client) => {
    ...
    if (existing.rowCount) {
        const dup = await client.query(`SELECT 1 FROM company_members WHERE company_id = $1 AND user_id = $2`, [companyId, existing.rows[0].id]);
        if (dup.rowCount)
    ...
    const profile = await client.query(`INSERT INTO worker_profiles (company_id, member_id, employment_status) VALUES ($1,$2,$3) RETURNING id`, [companyId, member.rows[0].id, body.status]);
    for (const s of body.specialties.map((x) => x.trim()).filter(Boolean)) {
        await client.query(`INSERT INTO worker_specialties (company_id, worker_profile_id, name) VALUES ($1,$2,$3)`, [companyId, profile.rows[0].id, s]);
    }
    for (let d = 0; d < 5; d++) {
        await client.query(`INSERT INTO worker_availability (company_id, worker_profile_id, weekday, start_time, end_time)
       VALUES ($1,$2,$3,'08:00','17:00')`, [companyId, profile.rows[0].id, d]);
    }
    await notify(client, {
...
workersRouter.patch('/:id', requireRole('admin'), requirePermission('workers.manage'), tenantRoute(async (req, res, client) => {
    ...
    if (body.name || body.phone !== undefined) {
        await client.query(`UPDATE users SET name = coalesce($2, name), phone = coalesce($3, phone) WHERE id = $1`, [req.params.id, body.name ?? null, body.phone ?? null]);
    }
    if (body.status) {
        const prev = (await client.query(`SELECT employment_status FROM worker_profiles WHERE id = $1`, [pid])).rows[0]?.employment_status;
        const memberRow = await client.query(`SELECT status FROM company_members WHERE company_id = $1 AND user_id = $2 AND role = 'field_worker'`, [companyId, req.params.id]);
        ...
        await client.query(`UPDATE worker_profiles SET employment_status = $2 WHERE id = $1`, [pid, body.status]);
        if (body.status === 'inactive') {
            await client.query(`UPDATE company_members SET status = 'inactive' WHERE user_id = $1 AND company_id = $2`, [req.params.id, companyId]);
        }
        else {
            await client.query(`UPDATE company_members SET status = 'active' WHERE user_id = $1 AND company_id = $2 AND role = 'field_worker'`, [req.params.id, companyId]);
        }
    }
    if (body.specialties) {
        await client.query(`DELETE FROM worker_specialties WHERE worker_profile_id = $1`, [pid]);
        for (const s of body.specialties) {
            await client.query(`INSERT INTO worker_specialties (company_id, worker_profile_id, name) VALUES ($1,$2,$3)`, [companyId, pid, s]);
        }
    }
...
workersRouter.delete('/:id', requireRole('admin'), requirePermission('workers.manage'), tenantRoute(async (req, res, client) => {
    const companyId = req.auth.companyId;
    const r = await client.query(`UPDATE company_members SET status = 'inactive' WHERE company_id = $1 AND user_id = $2 AND role = 'field_worker' RETURNING id`, [companyId, req.params.id]);
    if (!r.rowCount)
        throw notFound('Worker');
    await client.query(`UPDATE worker_profiles SET employment_status = 'inactive' WHERE member_id = $1`, [r.rows[0].id]);
    await audit(client, {
...
workersRouter.put('/:id/availability', requireRole('admin'), requirePermission('workers.manage'), tenantRoute(async (req, res, client) => {
    ...
    for (let i = 0; i < DAYS.length; i++) {
        ...
        if (slot) {
            await client.query(`INSERT INTO worker_availability (company_id, worker_profile_id, weekday, start_time, end_time)
         VALUES ($1,$2,$3,$4,$5)`, [companyId, pid, i, slot.start, slot.end]);
        }
    }
    res.json(await mapWorker(client, companyId, pid));
}));
workersRouter.post('/:id/time-off', requireRole('admin'), requirePermission('workers.manage'), tenantRoute(async (req, res, client) => {
    ...
    if (!pid)
        throw notFound('Worker');
    await client.query(`INSERT INTO worker_time_off (company_id, worker_profile_id, off_date) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING`, [companyId, pid, body.date]);
    res.json(await mapWorker(client, companyId, pid));
}));
workersRouter.delete('/:id/time-off/:date', requireRole('admin'), requirePermission('workers.manage'), tenantRoute(async (req, res, client) => {
    ...
    if (!pid)
        throw notFound('Worker');
    await client.query(`DELETE FROM worker_time_off WHERE worker_profile_id = $1 AND off_date = $2`, [pid, req.params.date]);
    res.json(await mapWorker(client, companyId, pid));
}));
//# sourceMappingURL=workers.routes.js.map