...
companyRouter.get('/', tenantRoute(async (req, res, client) => {
    ...
    const smtp = await client.query(`SELECT smtp_host, smtp_port, smtp_user, smtp_secure, smtp_from_name, smtp_from_email, smtp_reply_to, smtp_password_enc,
            twilio_account_sid, twilio_auth_token_enc, twilio_from_number
     FROM company_settings WHERE company_id = $1`, [req.auth.companyId]);
    const s = smtp.rows[0];
    const c = rows[0];
...
companyRouter.post('/onboarding/complete', requirePermission('settings.company'), tenantRoute(async (req, res, client) => {
    const { rows } = await client.query(`SELECT name, email, phone, address, timezone FROM companies WHERE id = $1`, [req.auth.companyId]);
    if (!rows[0])
        throw notFound('Company');
    if (!isCompanyProfileComplete(rows[0])) {
        throw badRequest('Complete required company information before finishing setup');
    }
    await client.query(`UPDATE companies SET onboarding_completed_at = COALESCE(onboarding_completed_at, now()) WHERE id = $1`, [req.auth.companyId]);
...
companyRouter.patch('/smtp', requirePermission('settings.smtp'), tenantRoute(async (req, res, client) => {
    ...
    await client.query(`INSERT INTO company_settings (company_id) VALUES ($1) ON CONFLICT (company_id) DO NOTHING`, [companyId]);
    const passwordEnc = body.password ? encryptSecret(body.password) : null;
    await client.query(`UPDATE company_settings SET
...
companyRouter.post('/smtp/test', requirePermission('settings.smtp'), requireTenant, async (req, res, next) => {
    try {
        const auth = req.auth;
        const companyId = auth.companyId;
        const { rows } = await pool.query(`SELECT status, trial_ends_at FROM companies WHERE id = $1 AND deleted_at IS NULL`, [companyId]);
...
companyRouter.patch('/twilio', requirePermission('settings.company'), tenantRoute(async (req, res, client) => {
    ...
    if (!from)
        throw badRequest('Enter a valid Twilio From number in E.164 format, e.g. +15551234567');
    const token = (body.authToken || '').trim();
    if (token && token.length < 8)
        throw badRequest('Auth token is too short');
    await client.query(`INSERT INTO company_settings (company_id) VALUES ($1) ON CONFLICT (company_id) DO NOTHING`, [companyId]);
    const existing = await client.query(`SELECT twilio_auth_token_enc FROM company_settings WHERE company_id = $1`, [companyId]);
    ...
    await client.query(`UPDATE company_settings SET
     ...
     WHERE company_id = $1`, [companyId, body.accountSid, tokenEnc, from]);
...
companyRouter.post('/twilio/test', requirePermission('settings.company'), requireTenant, async (req, res, next) => {
    try {
        ...
        if (!creds)
            throw badRequest('Twilio is not configured. Save Account SID, Auth Token, and From number first.');
