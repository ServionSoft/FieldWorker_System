...
commsRouter.get('/', tenantRoute(async (req, res, client) => {
    ...
    const { rows } = await client.query(`SELECT c.*, u.name AS user_name
...
commsRouter.post('/sms', tenantRoute(async (req, res, client) => {
    ...
    if (!to)
        throw badRequest('Invalid destination phone number');
...
commsRouter.post('/send-template', tenantRoute(async (req, res, client) => {
    ...
    const companyId = req.auth.companyId;
    const tpl = await client.query(`SELECT * FROM email_templates WHERE company_id = $1 AND id = $2`, [companyId, body.templateId]);
    if (!tpl.rowCount)
        throw notFound('Template');
    const type = z.enum(TENANT_TEMPLATE_TYPES).parse(tpl.rows[0].type);
    const cust = await client.query(`SELECT c.id, cc.email, cc.first_name, cc.last_name, cc.phone
     FROM customers c
     LEFT JOIN LATERAL (
       SELECT * FROM customer_contacts WHERE customer_id = c.id ORDER BY is_primary DESC LIMIT 1
     ) cc ON true
     WHERE c.company_id = $1 AND c.id = $2`, [companyId, body.customerId]);
    if (!cust.rowCount)
...
commsRouter.post('/email', tenantRoute(async (req, res, client) => {
    ...
    let estimateId = body.estimateId;
    if (!customerId && jobId) {
        const job = await client.query(`SELECT customer_id FROM jobs WHERE company_id = $1 AND id = $2`, [companyId, jobId]);
        if (!job.rowCount)
            throw notFound('Job');
        customerId = job.rows[0].customer_id;
    }
    if (!customerId && estimateId) {
        const est = await client.query(`SELECT customer_id FROM estimates WHERE company_id = $1 AND id = $2`, [companyId, estimateId]);
        if (!est.rowCount)
            throw notFound('Estimate');
    ...
    if (!to)
        throw badRequest('This customer has no email address');
    const attachments = [];
    if (body.invoiceId) {
        const inv = await client.query(`SELECT id, customer_id FROM invoices WHERE company_id = $1 AND id = $2`, [companyId, body.invoiceId]);
        if (!inv.rowCount || inv.rows[0].customer_id !== customerId)
            throw notFound('Invoice');
        const pdf = await buildInvoicePdf(client, companyId, body.invoiceId);
        if (pdf)
            attachments.push(pdf);
    ...
    if (attachEstimateId) {
        const est = await client.query(`SELECT id, customer_id FROM estimates WHERE company_id = $1 AND id = $2`, [companyId, attachEstimateId]);
        if (!est.rowCount || est.rows[0].customer_id !== customerId)
            throw notFound('Estimate');
...
commsRouter.post('/:id/read', tenantRoute(async (req, res, client) => {
    const r = await client.query(`UPDATE communications SET read = true WHERE company_id = $1 AND id = $2 RETURNING id`, [req.auth.companyId, req.params.id]);
    if (!r.rowCount)
        throw notFound('Communication');
    res.json({ ok: true });
}));
commsRouter.post('/read-all', tenantRoute(async (req, res, client) => {
    ...
    await client.query(`UPDATE communications SET read = true
     WHERE company_id = $1
       AND ($2::uuid IS NULL OR customer_id = $2)
       AND ($3::uuid IS NULL OR job_id = $3)
       AND ($4::uuid IS NULL OR estimate_id = $4)`, [req.auth.companyId, filter.customerId ?? null, filter.jobId ?? null, filter.estimateId ?? null]);
    res.json({ ok: true });
}));
