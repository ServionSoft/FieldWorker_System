...
import { PLAN_FEATURE_KEYS } from '../billing/plan-features.js';
import { applyPaymentFailed, applyPaymentPaid, applySubscriptionSuspended, simulationInvoice, } from '../billing/billing-effects.js';
const featureKeysSchema = z.array(z.enum(PLAN_FEATURE_KEYS));
...
platformRouter.get('/companies', platformRoute(async (req, res, client) => {
    const { rows } = await client.query(`SELECT c.*, p.name AS plan_name,
...
platformRouter.post('/companies', platformRoute(async (req, res, client) => {
    ...
    const created = await client.query(`INSERT INTO companies (name, email, phone, address, plan_id, status, trial_ends_at)
     VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`, [body.name, body.email, body.phone, body.address, body.planId, body.status, body.status === 'trial' ? trialEnds.toISOString() : null]);
    ...
    const inv = await client.query(`INSERT INTO invitations (company_id, email, name, role, token_hash, invited_by, expires_at)
     VALUES ($1,$2,$3,'owner',$4,$5, now() + interval '7 days') RETURNING id`, [created.rows[0].id, ownerEmail, body.ownerName ?? body.name, sha256(token), req.auth.userId]);
...
platformRouter.patch('/companies/:id', platformRoute(async (req, res, client) => {
    ...
    const r = await client.query(`UPDATE companies SET
     ...
     WHERE id = $2 AND $1::text IS NOT NULL AND deleted_at IS NULL RETURNING *`, ['x', req.params.id, body.name ?? null, body.email ?? null, body.phone ?? null, body.address ?? null, body.planId ?? null, body.status ?? null]);
    if (!r.rowCount)
...
platformRouter.delete('/companies/:id', platformRoute(async (req, res, client) => {
    const r = await client.query(`UPDATE companies SET deleted_at = now(), status = 'suspended' WHERE id = $1 AND deleted_at IS NULL RETURNING id`, [req.params.id]);
    if (!r.rowCount)
        throw notFound('Company');
    res.json({ ok: true });
}));
...
platformRouter.get('/dashboard', platformRoute(async (_req, res, client) => {
    ...
    const employees = await client.query(`SELECT count(*)::int AS n FROM company_members m JOIN companies c ON c.id = m.company_id WHERE c.deleted_at IS NULL AND m.status = 'active'`);
    const activity = await client.query(`SELECT action, created_at, metadata FROM audit_logs ORDER BY created_at DESC LIMIT 10`);
    let mrr = 0;
...
platformRouter.get('/reports', platformRoute(async (_req, res, client) => {
    const companies = await client.query(`SELECT date_trunc('month', created_at) AS month, count(*)::int AS n
     FROM companies WHERE deleted_at IS NULL GROUP BY 1 ORDER BY 1`);
    const jobs = await client.query(`SELECT category, count(*)::int AS n FROM jobs GROUP BY category`);
    res.json({
...
platformRouter.get('/companies/:id', platformRoute(async (req, res, client) => {
    const { rows } = await client.query(`SELECT c.*, p.name AS plan_name FROM companies c JOIN subscription_plans p ON p.id = c.plan_id WHERE c.id = $1`, [req.params.id]);
    if (!rows[0])
        throw notFound('Company');
    const c = rows[0];
    const members = await client.query(`SELECT m.id, m.role, m.status, u.id AS user_id, u.name, u.email
     FROM company_members m JOIN users u ON u.id = m.user_id
     WHERE m.company_id = $1 ORDER BY m.created_at`, [c.id]);
    const invoices = await client.query(`SELECT * FROM billing_invoices WHERE company_id = $1 ORDER BY created_at DESC LIMIT 25`, [c.id]);
    const invites = await client.query(`SELECT id, email, role, expires_at, accepted_at FROM invitations WHERE company_id = $1 ORDER BY created_at DESC LIMIT 10`, [c.id]);
    res.json({
        id: c.id, name: c.name, email: c.email, phone: c.phone ?? '', address: c.address ?? '',
...
platformRouter.post('/companies/:id/invite-owner', platformRoute(async (req, res, client) => {
    ...
    const token = randomToken();
    await client.query(`INSERT INTO invitations (company_id, email, name, role, token_hash, invited_by, expires_at)
     VALUES ($1,$2,$3,'owner',$4,$5, now() + interval '7 days')`, [req.params.id, body.email, body.name ?? null, sha256(token), req.auth.userId]);
    const link = `${(env.APP_PUBLIC_URL || env.CORS_ORIGIN).replace(/\/$/, '')}/invite/${token}`;
    await sendPlatformEmail({
        to: body.email,
        templateType: 'tenant_invitation',
...
platformRouter.post('/companies/:id/impersonate', platformRoute(async (req, res, client) => {
    const { rows } = await client.query(`SELECT m.id, m.user_id, m.role, u.name, u.email
     FROM company_members m JOIN users u ON u.id = m.user_id
    ...
    if (!rows[0])
        throw badRequest('Company has no owner or admin to impersonate');
    const m = rows[0];
...
platformRouter.post('/plans', platformRoute(async (req, res, client) => {
    ...
    const r = await client.query(`INSERT INTO subscription_plans (name, price_cents, price_cents_yearly, billing_interval, max_workers, max_jobs, features, feature_keys, stripe_price_id_monthly, stripe_price_id_yearly)
...
platformRouter.patch('/plans/:id', platformRoute(async (req, res, client) => {
    ...
    const r = await client.query(`UPDATE subscription_plans SET
...
platformRouter.patch('/settings', platformRoute(async (req, res, client) => {
    ...
    await client.query(`UPDATE platform_settings SET
...
platformRouter.patch('/email-templates/:type', platformRoute(async (req, res, client) => {
    ...
    const r = await client.query(`UPDATE platform_email_templates SET
       name = coalesce($2, name), subject = coalesce($3, subject), body = coalesce($4, body), updated_at = now()
     WHERE type = $1 RETURNING type, name, subject, body`, [type, body.name ?? null, body.subject ?? null, body.body ?? null]);
    if (!r.rowCount)
...
platformRouter.get('/audit', platformRoute(async (req, res, client) => {
    ...
    const { rows } = await client.query(`SELECT a.*, u.email AS actor_email, c.name AS company_name
     FROM audit_logs a
...
platformRouter.post('/companies/:id/billing-test', platformRoute(async (req, res, client) => {
    ...
    const company = await client.query(`SELECT c.id, c.name, c.status, p.price_cents
     FROM companies c JOIN subscription_plans p ON p.id = c.plan_id
    ...
    if (body.action === 'payment_failed') {
        ...
        const open = await client.query(`SELECT stripe_invoice_id, number, amount_cents, currency
       ...
       ORDER BY created_at DESC LIMIT 1`, [c.id]);
        ...
        await applyPaymentPaid(c.id, existing
            ? {
            ...
            : simulationInvoice(amountCents, 'paid'), client);
    }
    ...
    const invoices = await client.query(`SELECT * FROM billing_invoices WHERE company_id = $1 ORDER BY created_at DESC LIMIT 10`, [c.id]);
...
platformRouter.get('/billing-invoices', platformRoute(async (req, res, client) => {
    const { page, pageSize, offset } = parsePage(req.query);
    const { rows } = await client.query(`SELECT b.*, c.name AS company_name
