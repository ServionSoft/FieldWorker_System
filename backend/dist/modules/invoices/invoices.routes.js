...
import { nextNumber, parsePage } from '../../utils/helpers.js';
...
async function mapInvoice(client, companyId, id) {
    const { rows } = await client.query(`SELECT i.*,
    ...
    if (!i)
        return null;
    const items = (await client.query(`SELECT description, quantity, unit_price, total FROM invoice_line_items WHERE invoice_id = $1`, [id])).rows.map((r) => ({
...
invoicesRouter.get('/', tenantRoute(async (req, res, client) => {
    ...
    const { rows } = await client.query(`SELECT i.id
     FROM invoices i
     LEFT JOIN LATERAL (
    ...
    const items = [];
    for (const r of rows)
        items.push(await mapInvoice(client, companyId, r.id));
