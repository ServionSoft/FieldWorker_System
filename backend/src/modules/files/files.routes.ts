...
function authOrSigned(kind: 'file' | 'document') {
...
filesRouter.post('/', requireRole('admin', 'field_worker'), upload.single('file'), tenantRoute(async (req, res, client) => {
  ...
  const created = await client.query(
    `INSERT INTO files (company_id, storage_key, mime_type, size_bytes, original_name, uploaded_by)
     VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
    [companyId, storageKey, req.file.mimetype, req.file.size, req.file.originalname, req.auth.userId],
  );
  res.status(201).json({
...
filesRouter.get('/:id/download', tenantRoute(async (req, res, client) => {
  const { rows } = await client.query(
    `SELECT storage_key, original_name, mime_type FROM files WHERE company_id = $1 AND id = $2`,
    [req.auth.companyId, req.params.id],
  );
  if (!rows[0]) throw notFound('File');
  const full = path.resolve(uploadDir, rows[0].storage_key);
  if (!full.startsWith(path.resolve(uploadDir))) throw notFound('File');
...
filesRouter.post('/:id/signed-url', requireRole('admin', 'field_worker'), tenantRoute(async (req, res, client) => {
  ...
  if (!file.rowCount) throw notFound('File');
...
documentsRouter.get('/', tenantRoute(async (req, res, client) => {
  ...
  const { rows } = await client.query(
    `SELECT d.id, d.job_id, d.created_at, f.original_name, f.mime_type, f.size_bytes, u.name AS uploaded_by
     FROM documents d
...
documentsRouter.post('/', tenantRoute(async (req, res, client) => {
  ...
  const jobId = (req.body.jobId as string) || null;
  ...
  if (!file.rowCount) throw notFound('File');
  const created = await client.query(
    `INSERT INTO documents (company_id, file_id, job_id, uploaded_by) VALUES ($1,$2,$3,$4) RETURNING id`,
    [req.auth.companyId, fileId, jobId, req.auth.userId],
  );
...
documentsRouter.get('/:id/download', tenantRoute(async (req, res, client) => {
  ...
  const full = path.resolve(uploadDir, rows[0].storage_key);
  if (!full.startsWith(path.resolve(uploadDir))) throw notFound('Document');
  res.setHeader('Content-Type', rows[0].mime_type ?? 'application/octet-stream');
...
documentsRouter.delete('/:id', requireRole('admin'), tenantRoute(async (req, res, client) => {
  const r = await client.query(
    `DELETE FROM documents WHERE company_id = $1 AND id = $2 RETURNING id`,
    [req.auth.companyId, req.params.id],
  );
  if (!r.rowCount) throw notFound('Document');
  await audit(client, {
...
jobImagesRouter.post('/', tenantRoute(async (req, res, client) => {
  ...
  if (req.auth.role === 'field_worker') {
    const ok = await client.query(
      ...
      [req.params.id, req.auth.memberId],
    );
    if (!ok.rowCount) throw notFound('Job');
  }
  ...
  await client.query(
    `INSERT INTO job_images (company_id, job_id, file_id) VALUES ($1,$2,$3)`,
    [req.auth.companyId, req.params.id, fileId],
  );
