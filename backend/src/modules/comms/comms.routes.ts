...
commsRouter.post('/send-template', tenantRoute(async (req, res, client) => {
  ...
  if (body.invoiceId) {
    const inv = await client.query(
      `SELECT id, invoice_number, total, due_date, job_id, customer_id
       FROM invoices WHERE company_id = $1 AND id = $2`,
      [companyId, body.invoiceId],
    );
    if (!inv.rowCount || inv.rows[0].customer_id !== body.customerId) throw notFound('Invoice');
  ...
  if (jobId) {
    const job = await client.query(
      `SELECT id, title, scheduled_date, scheduled_time, customer_id
       FROM jobs WHERE company_id = $1 AND id = $2`,
      [companyId, jobId],
    );
    if (!job.rowCount || job.rows[0].customer_id !== body.customerId) throw notFound('Job');
  ...
  if (estimateId) {
    const est = await client.query(
      `SELECT id, estimate_number, customer_id FROM estimates WHERE company_id = $1 AND id = $2`,
      [companyId, estimateId],
    );
    if (!est.rowCount || est.rows[0].customer_id !== body.customerId) throw notFound('Estimate');
    extra.estimateNumber = String(est.rows[0].estimate_number ?? '');
  }
  if (body.followUpId) {
    const fu = await client.query(
      `SELECT id, title, due_date, job_id, estimate_id, customer_id
       FROM follow_ups WHERE company_id = $1 AND id = $2`,
      [companyId, body.followUpId],
    );
    if (!fu.rowCount || fu.rows[0].customer_id !== body.customerId) throw notFound('Follow-up');
