...
function okTwiml(res: import('express').Response, xml = '<Response></Response>') {
  res.type('text/xml').send(xml);
}

async function findCompanyByNumber(phone: string) {
...
async function findCustomerByPhone(companyId: string, phone: string) {
  ...
  return withTenant(companyId, null, false, async (client) => {
    ...
    return (rows[0]?.customer_id as string) ?? null;
  });
}

async function authTokensForRequest(req: import('express').Request) {
  const tokens = new Set<string>();
  const companyIds = new Set<string>();
  const to = String(req.body?.To ?? '');
  const companyFromTo = await findCompanyByNumber(to);
  if (companyFromTo) companyIds.add(companyFromTo);
  const sid = String(req.body?.CallSid || req.body?.MessageSid || '');
  if (sid) {
    const { rows } = await adminPool.query(
      `SELECT company_id FROM communications WHERE twilio_sid = $1 LIMIT 1`,
      [sid],
    );
    if (rows[0]?.company_id) companyIds.add(rows[0].company_id);
  }
  const connectMatch = String(req.originalUrl || req.url).match(/voice\/connect\/([0-9a-f-]{36})/i);
  if (connectMatch) {
    const { rows } = await adminPool.query(
      `SELECT company_id FROM communications WHERE id = $1`,
      [connectMatch[1]],
    );
    if (rows[0]?.company_id) companyIds.add(rows[0].company_id);
  }
  ...
  if (env.TWILIO_AUTH_TOKEN) tokens.add(env.TWILIO_AUTH_TOKEN);
  return [...tokens];
}

twilioWebhooks.use(async (req, res, next) => {
  if (env.TWILIO_SKIP_SIGNATURE) return next();
  try {
    const tokens = await authTokensForRequest(req);
    if (tokens.some((token) => requestMatchesTwilioToken(req as any, token))) return next();
  } catch {
    /* reject below */
  }
      ...
      if (!existing.rowCount) {
        await client.query(
          `INSERT INTO communications (
           ...
           ) VALUES ($1,'sms','inbound','received',$2,$3,$4,$5,$6,false)`,
          [companyId, from, to, customerId, body || null, sid || null],
        );
      }
    });
  }
      ...
      if (!existing.rowCount) {
        await client.query(
          `INSERT INTO communications (
             company_id, type, direction, status, from_number, to_number,
             customer_id, body, twilio_sid, read
           ) VALUES ($1,'call','inbound','queued',$2,$3,$4,$5,$6,false)`,
          [companyId, from, to, customerId, 'Inbound call', sid || null],
        );
      }
    });
  }
...
twilioWebhooks.post('/voice/connect/:id', async (req, res) => {
  const { rows } = await adminPool.query(
    `SELECT to_number, from_number FROM communications WHERE id = $1`,
    [req.params.id],
  );
  const vr = new twiml.VoiceResponse();
  if (!rows[0]) {
...
twilioWebhooks.post('/status', async (req, res) => {
  ...
  if (sid) {
    const mapped =
      status === 'completed' || status === 'delivered' ? (req.body.CallSid ? 'completed' : 'delivered')
        : status === 'busy' ? 'busy'
    ...
    await adminPool.query(
      `UPDATE communications SET
       ...
       WHERE twilio_sid = $1`,
      [sid, mapped, Number.isFinite(duration) ? duration : null],
    );
  }
  res.status(204).end();
});
