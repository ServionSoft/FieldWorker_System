...
async function assertCustomer(client: import('pg').PoolClient, companyId: string, id: string) {
...
function customerId(req: { params: Record<string, string> }) {
...
function mapAddress(r: Record<string, unknown>) {
  return {
    ...
    formatted: formatAddress(r as any),
  };
}
...
customerNestedRouter.post('/contacts', requirePermission('customers.write'), tenantRoute(async (req, res, client) => {
  ...
  const created = await client.query(
    ...
    [companyId, id, body.firstName, body.lastName, body.phone, body.phoneExt ?? null, body.email || null, body.isPrimary ?? false],
  );
  await audit(client, {
...
customerNestedRouter.delete('/contacts/:contactId', requirePermission('customers.write'), tenantRoute(async (req, res, client) => {
  ...
  const id = customerId(req);
  const r = await client.query(
    `DELETE FROM customer_contacts WHERE company_id = $1 AND customer_id = $2 AND id = $3 AND is_primary = false RETURNING id`,
    [companyId, id, req.params.contactId],
  );
  if (!r.rowCount) throw badRequest('Cannot delete primary contact (or not found)');
  res.json({ ok: true });
}));
...
customerNestedRouter.post('/addresses', requirePermission('customers.write'), tenantRoute(async (req, res, client) => {
  ...
  const created = await client.query(
    ...
    [companyId, id, body.locationName, body.street, body.unit ?? null, body.city, body.state, body.zip, body.gatedProperty, body.isDefault ?? false],
  );
  await audit(client, {
...
customerNestedRouter.patch('/addresses/:addressId', requirePermission('customers.write'), tenantRoute(async (req, res, client) => {
  ...
  if (!r.rowCount) throw notFound('Address');
...
customerNestedRouter.delete('/addresses/:addressId', requirePermission('customers.write'), tenantRoute(async (req, res, client) => {
  ...
  const id = customerId(req);
  const r = await client.query(
    `DELETE FROM addresses WHERE company_id = $1 AND customer_id = $2 AND id = $3 AND is_default = false RETURNING id`,
    [companyId, id, req.params.addressId],
  );
  if (!r.rowCount) throw badRequest('Cannot delete default address (or not found)');
  res.json({ ok: true });
}));
...
customerNestedRouter.get('/notes', tenantRoute(async (req, res, client) => {
  ...
  await assertCustomer(client, companyId, id);
  const { rows } = await client.query(
    `SELECT n.id, n.body, n.created_at, u.name AS author_name, n.author_id
...
customerNestedRouter.post('/notes', requirePermission('customers.write'), tenantRoute(async (req, res, client) => {
  ...
  const created = await client.query(
    `INSERT INTO customer_notes (company_id, customer_id, author_id, body) VALUES ($1,$2,$3,$4) RETURNING *`,
    [companyId, id, req.auth.userId, body.body],
  );
  await audit(client, {
...
customerNestedRouter.delete('/notes/:noteId', requirePermission('customers.write'), tenantRoute(async (req, res, client) => {
  const r = await client.query(
    `DELETE FROM customer_notes WHERE company_id = $1 AND customer_id = $2 AND id = $3 RETURNING id`,
    [req.auth.companyId, customerId(req), req.params.noteId],
  );
  if (!r.rowCount) throw notFound('Note');
  res.json({ ok: true });
}));
...
customerNestedRouter.get('/activity', tenantRoute(async (req, res, client) => {
  ...
  const id = customerId(req);
  await assertCustomer(client, companyId, id);
  const items: { at: string; kind: string; title: string; detail?: string; href?: string }[] = [];

  const jobs = await client.query(
    `SELECT id, title, status, created_at FROM jobs WHERE company_id = $1 AND customer_id = $2`,
    [companyId, id],
  );
  ...
  const estimates = await client.query(
    `SELECT id, estimate_number, status, total, created_at FROM estimates WHERE company_id = $1 AND customer_id = $2`,
    [companyId, id],
  );
  ...
  const invoices = await client.query(
    `SELECT id, invoice_number, status, total, created_at, paid_at FROM invoices WHERE company_id = $1 AND customer_id = $2`,
    [companyId, id],
  );
  ...
  const followUps = await client.query(
    `SELECT title, due_date, done, created_at FROM follow_ups WHERE company_id = $1 AND customer_id = $2`,
    [companyId, id],
  );
  ...
  const comms = await client.query(
    `SELECT type, direction, body, created_at FROM communications WHERE company_id = $1 AND customer_id = $2`,
    [companyId, id],
  );
  ...
  const notes = await client.query(
    `SELECT body, created_at FROM customer_notes WHERE company_id = $1 AND customer_id = $2`,
    [companyId, id],
  );
  ...
  const audits = await client.query(
    `SELECT action, created_at FROM audit_logs WHERE company_id = $1 AND entity_type = 'customer' AND entity_id = $2`,
    [companyId, id],
  );
