...
async function officeUserIds(client: import('pg').PoolClient, companyId: string) {
  const { rows } = await client.query(
    `SELECT user_id FROM company_members
     WHERE company_id = $1 AND status = 'active' AND role IN (${OFFICE_SQL})
     ORDER BY CASE role WHEN 'owner' THEN 0 WHEN 'admin' THEN 1 WHEN 'dispatcher' THEN 2 ELSE 3 END`,
    [companyId],
  );
  return rows.map((r: { user_id: string }) => r.user_id as string);
}

async function ensureWorkerOfficeThread(client: import('pg').PoolClient, companyId: string, workerUserId: string) {
  ...
  let threadId = existing.rows[0]?.id as string | undefined;
  if (!threadId) {
    ...
    await client.query(
      `INSERT INTO chat_thread_participants (company_id, thread_id, user_id) VALUES ($1,$2,$3)`,
      [companyId, threadId, workerUserId],
    );
  }

  const officeIds = await officeUserIds(client, companyId);
  for (const uid of officeIds) {
    if (uid === workerUserId) continue;
    await client.query(
      `INSERT INTO chat_thread_participants (company_id, thread_id, user_id) VALUES ($1,$2,$3)
       ON CONFLICT DO NOTHING`,
      [companyId, threadId, uid],
    );
  }
  return threadId;
}

async function ensureDirectThread(client: import('pg').PoolClient, companyId: string, userA: string, userB: string) {
  if (userA === userB) throw notFound('User');
  const existing = await client.query(
    `SELECT t.id FROM chat_threads t
     WHERE t.company_id = $1
  ...
  await client.query(
    ...
    [companyId, t.rows[0].id, userA, userB],
  );
  return t.rows[0].id as string;
}

async function assertParticipant(client: import('pg').PoolClient, threadId: string, userId: string) {
  const { rows } = await client.query(
    `SELECT 1 FROM chat_thread_participants WHERE thread_id = $1 AND user_id = $2`,
    [threadId, userId],
  );
  if (!rows[0]) throw notFound('Thread');
}
...
chatRouter.get('/threads', tenantRoute(async (req, res, client) => {
  ...
  for (const t of rows) {
    const parts = await client.query(
      `SELECT p.user_id, u.name FROM chat_thread_participants p JOIN users u ON u.id = p.user_id WHERE p.thread_id = $1`,
      [t.id],
    );
    const last = await client.query(
      `SELECT id, sender_id, body, created_at FROM chat_messages WHERE thread_id = $1 ORDER BY created_at DESC LIMIT 1`,
      [t.id],
    );
    const unread = await client.query(
      `SELECT count(*)::int AS n FROM chat_messages m
       WHERE m.thread_id = $1 AND m.sender_id <> $2
         AND NOT EXISTS (SELECT 1 FROM chat_message_reads r WHERE r.message_id = m.id AND r.user_id = $2)`,
...
chatRouter.post('/threads', tenantRoute(async (req, res, client) => {
  ...
  if (!body.userId) throw notFound('User');
  const other = await client.query(
    `SELECT user_id, role FROM company_members WHERE company_id = $1 AND user_id = $2 AND status = 'active'`,
    [companyId, body.userId],
  );
  if (!other.rowCount) throw notFound('User');
  const otherRole = other.rows[0].role as string;
...
chatRouter.post('/threads/:id/messages', tenantRoute(async (req, res, client) => {
  ...
  const created = await client.query(
    `INSERT INTO chat_messages (company_id, thread_id, sender_id, body) VALUES ($1,$2,$3,$4) RETURNING *`,
    [req.auth.companyId, req.params.id, req.auth.userId, body.message.trim()],
  );
  await client.query(
    `INSERT INTO chat_message_reads (message_id, user_id) VALUES ($1,$2)`,
    [created.rows[0].id, req.auth.userId],
  );
  const payload = {
  ...
  const parts = await client.query(
    `SELECT user_id FROM chat_thread_participants WHERE thread_id = $1 AND user_id <> $2`,
    [req.params.id, req.auth.userId],
  );
