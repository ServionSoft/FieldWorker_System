...
inventoryRouter.get('/', tenantRoute(async (req, res, client) => {
  const { rows } = await client.query(
    `SELECT * FROM inventory_items WHERE company_id = $1 ORDER BY name`,
    [req.auth.companyId],
  );
...
inventoryRouter.post('/', tenantRoute(async (req, res, client) => {
  ...
  try {
    const created = await client.query(
      `INSERT INTO inventory_items (company_id, name, sku, category, quantity, min_stock, unit_price)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [companyId, body.name, body.sku, body.category, body.quantity, body.minStock, body.unitPrice],
    );
    if (body.quantity) {
      await client.query(
        `INSERT INTO inventory_movements (company_id, item_id, type, quantity_delta, created_by)
         VALUES ($1,$2,'receive',$3,$4)`,
        [companyId, created.rows[0].id, body.quantity, req.auth.userId],
      );
    }
...
inventoryRouter.patch('/:id', tenantRoute(async (req, res, client) => {
  ...
  if (!existing.rowCount) throw notFound('Item');
  ...
  const updated = await client.query(
    ...
    [companyId, req.params.id, body.name ?? null, body.sku ?? null, body.category ?? null, body.quantity ?? null, body.minStock ?? null, body.unitPrice ?? null],
  );
  if (body.quantity !== undefined && body.quantity !== prev.quantity) {
    await client.query(
      `INSERT INTO inventory_movements (company_id, item_id, type, quantity_delta, created_by)
       VALUES ($1,$2,'adjust',$3,$4)`,
      [companyId, req.params.id, body.quantity - prev.quantity, req.auth.userId],
    );
  }
  const item = updated.rows[0];
  if (item.quantity < item.min_stock) {
    await notifyCompanyAdmins(
      client, companyId, 'Low Stock Alert',
      `${item.name} is below minimum stock level (${item.quantity}/${item.min_stock})`,
      'warning',
      { eventKey: 'inventory.low_stock', entityType: 'inventory', entityId: req.params.id, linkPath: '/admin/inventory' },
    );
  }
...
inventoryRouter.delete('/:id', tenantRoute(async (req, res, client) => {
  await client.query(
    `DELETE FROM inventory_movements WHERE company_id = $1 AND item_id = $2`,
