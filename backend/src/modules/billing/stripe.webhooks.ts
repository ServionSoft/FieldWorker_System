...
async function companyByInvoiceId(invoiceId: string) {
...
function invoiceSubscriptionId(inv: Stripe.Invoice) {
  ...
  try {
    if (event.type === 'checkout.session.completed') {
      ...
      if (companyId) {
        if (session.subscription) {
          ...
          await pool.query(
            ...
            [companyId, typeof session.customer === 'string' ? session.customer : session.customer.id],
          );
        }
      ...
      const company = await companyByStripe(
        ...
        invoiceSubscriptionId(inv),
        inv.metadata?.companyId,
      ) || await companyByInvoiceId(inv.id);
      if (company) {
