...
import { assertPlanLimits, formatAddress, notify, notifyCompanyAdmins, parsePage, pageResult } from '../../utils/helpers.js';
...
async function memberIdForUser(client: import('pg').PoolClient, companyId: string, userId: string) {
...
async function memberWantsAssignmentEmail(client: import('pg').PoolClient, companyId: string, userId: string) {
  const { rows } = await client.query(
    `SELECT notify_email_assignments FROM company_members WHERE company_id = $1 AND user_id = $2`,
    [companyId, userId],
  );
  return !!rows[0]?.notify_email_assignments;
}

async function mapJob(client: import('pg').PoolClient, companyId: string, id: string, userId?: string) {
  const { rows } = await client.query(
    `SELECT j.*,
  ...
  const assignees = (await client.query(
    `SELECT m.user_id FROM job_assignees ja JOIN company_members m ON m.id = ja.member_id WHERE ja.job_id = $1`,
    [id],
  )).rows.map((r: { user_id: string }) => r.user_id);
  const notes = (await client.query(
    `SELECT body FROM job_notes WHERE job_id = $1 ORDER BY created_at`,
    [id],
  )).rows.map((r: { body: string }) => r.body);
  const materials = (await client.query(
    `SELECT name FROM job_materials WHERE job_id = $1`,
    [id],
  )).rows.map((r: { name: string }) => r.name);
  const tasks = (await client.query(
    `SELECT id, title, done FROM job_tasks WHERE job_id = $1`,
    [id],
  )).rows.map((r) => ({ id: r.id, title: r.title, done: r.done }));
  const lineItems = (await client.query(
    `SELECT id, group_name, description, warehouse, quantity, unit_price, total, taxable FROM job_line_items WHERE job_id = $1`,
    [id],
  )).rows.map((r) => ({
  ...
  const images = (await client.query(
    `SELECT f.id FROM job_images ji JOIN files f ON f.id = ji.file_id WHERE ji.job_id = $1`,
    [id],
...
async function assertAssigned(client: import('pg').PoolClient, jobId: string, memberId: string) {
  const { rows } = await client.query(
    `SELECT 1 FROM job_assignees WHERE job_id = $1 AND member_id = $2`,
    [jobId, memberId],
  );
...
jobsRouter.get('/', requireRole('admin', 'field_worker'), tenantRoute(async (req, res, client) => {
  ...
  const orderSql =
    sortBy === 'newest' ? 'j.created_at DESC'
  ...
  const items = [];
  for (const r of idRows) items.push(await mapJob(client, companyId, r.id, userId));
...
jobsRouter.post('/', requireRole('admin'), requirePermission('jobs.write'), tenantRoute(async (req, res, client) => {
  ...
  if (!addressId) {
    const addr = await client.query(
      `SELECT id FROM addresses WHERE company_id = $1 AND customer_id = $2 ORDER BY is_default DESC LIMIT 1`,
  ...
  for (const userId of body.assignedWorkerIds) {
    ...
    if (!mid) throw badRequest('Invalid worker');
    const leave = await client.query(
      `SELECT employment_status FROM worker_profiles WHERE member_id = $1`,
      [mid],
    );
    if (leave.rows[0]?.employment_status === 'on_leave') throw badRequest('Worker is on leave');
    await client.query(
      `INSERT INTO job_assignees (company_id, job_id, member_id) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING`,
      [companyId, id, mid],
    );
    await notify(client, {
  ...
  for (const m of body.materials) {
    await client.query(
      `INSERT INTO job_materials (company_id, job_id, name) VALUES ($1,$2,$3)`,
      [companyId, id, m],
    );
  }
  for (const li of body.lineItems) {
    await client.query(
      `INSERT INTO job_line_items (company_id, job_id, description, quantity, unit_price, total, taxable)
       VALUES ($1,$2,$3,$4,$5,$6,$7)`,
      [companyId, id, li.description, li.quantity, li.unitPrice, li.total ?? li.quantity * li.unitPrice, li.taxable ?? true],
    );
  }
...
jobsRouter.post('/bulk', requireRole('admin'), requirePermission('jobs.write'), tenantRoute(async (req, res, client) => {
  ...
  if (body.action === 'archive') {
    const r = await client.query(
      `UPDATE jobs SET archived_at = now(), archived_by = $3, updated_by = $3
    ...
    const r = await client.query(
      `UPDATE jobs SET archived_at = NULL, archived_by = NULL, updated_by = $3
    ...
    const r = await client.query(
      `UPDATE jobs SET status = $3, updated_by = $4,
...
jobsRouter.post('/:id/archive', requireRole('admin'), requirePermission('jobs.write'), tenantRoute(async (req, res, client) => {
  ...
  if (!r.rowCount) throw notFound('Job');
  await audit(client, {
...
jobsRouter.post('/:id/status', requireRole('admin', 'field_worker'), tenantRoute(async (req, res, client) => {
  ...
  if (req.auth.role === 'field_worker') {
    await assertAssigned(client, req.params.id, req.auth.memberId!);
    if (!['in_progress', 'completed'].includes(body.status)) throw badRequest('Invalid status transition');
  }
  const job = await mapJob(client, companyId, req.params.id);
  if (!job) throw notFound('Job');
  await client.query(
    `UPDATE jobs SET status = $3, completed_at = CASE WHEN $3 = 'completed' THEN now() ELSE completed_at END
     WHERE company_id = $1 AND id = $2`,
  ...
  if (body.status === 'completed') {
    ...
    for (const uid of job.assignedWorkerIds ?? []) {
      await client.query(
        `UPDATE worker_profiles wp SET jobs_completed = jobs_completed + 1
         FROM company_members m WHERE m.id = wp.member_id AND m.user_id = $1 AND wp.company_id = $2`,
        [uid, companyId],
      );
    }
...
jobsRouter.post('/:id/notes', requireRole('admin', 'field_worker'), tenantRoute(async (req, res, client) => {
  ...
  if (req.auth.role === 'field_worker') await assertAssigned(client, req.params.id, req.auth.memberId!);
  const job = await mapJob(client, req.auth.companyId!, req.params.id);
  if (!job) throw notFound('Job');
  await client.query(
    `INSERT INTO job_notes (company_id, job_id, author_id, body) VALUES ($1,$2,$3,$4)`,
    [req.auth.companyId, req.params.id, req.auth.userId, body.body],
  );
...
jobsRouter.put('/:id/line-items', requireRole('admin'), requirePermission('jobs.write'), tenantRoute(async (req, res, client) => {
  ...
  for (const li of body.items) {
    await client.query(
      `INSERT INTO job_line_items (company_id, job_id, description, quantity, unit_price, total, taxable)
       VALUES ($1,$2,$3,$4,$5,$6,$7)`,
      [companyId, req.params.id, li.description, li.quantity, li.unitPrice, li.total ?? li.quantity * li.unitPrice, li.taxable ?? true],
    );
  }
  res.json(await mapJob(client, companyId, req.params.id));
}));
