...
async function mapEstimate(client: import('pg').PoolClient, companyId: string, id: string) {
  const { rows } = await client.query(
    `SELECT e.*,
  ...
  const items = (await client.query(
    `SELECT description, quantity, unit_price, total FROM estimate_line_items WHERE estimate_id = $1`,
    [id],
  )).rows.map((r) => ({
    description: r.description, quantity: Number(r.quantity), unitPrice: Number(r.unit_price), total: Number(r.total),
  }));
  const assignedWorkerIds = (await client.query(
    `SELECT m.user_id FROM estimate_assignees ea JOIN company_members m ON m.id = ea.member_id WHERE ea.estimate_id = $1`,
    [id],
...
estimatesRouter.get('/', tenantRoute(async (req, res, client) => {
  ...
  const items = [];
  for (const r of rows) items.push(await mapEstimate(client, companyId, r.id));
...
estimatesRouter.post('/', requirePermission('estimates.write'), tenantRoute(async (req, res, client) => {
  ...
  for (const li of body.items) {
    await client.query(
      `INSERT INTO estimate_line_items (company_id, estimate_id, description, quantity, unit_price, total)
       VALUES ($1,$2,$3,$4,$5,$6)`,
      [companyId, id, li.description, li.quantity, li.unitPrice, li.total ?? li.quantity * li.unitPrice],
    );
  }
  for (const uid of body.assignedWorkerIds) {
    ...
    if (mid) {
      await client.query(
        `INSERT INTO estimate_assignees (company_id, estimate_id, member_id) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING`,
        [companyId, id, mid],
      );
    }
  }
...
estimatesRouter.get('/:id/pdf', tenantRoute(async (req, res, client) => {
  ...
  if (!pdf) throw notFound('Estimate');
  ...
  if (body.items) {
    ...
    for (const li of body.items) {
      await client.query(
        `INSERT INTO estimate_line_items (company_id, estimate_id, description, quantity, unit_price, total)
         VALUES ($1,$2,$3,$4,$5,$6)`,
        [companyId, req.params.id, li.description, li.quantity, li.unitPrice, li.total ?? li.quantity * li.unitPrice],
      );
    }
    await client.query(
      `UPDATE estimates SET subtotal=$3, tax=$4, total=$5, tax_rate=$6, notes=coalesce($7, notes), description=coalesce($8, description),
         valid_until = coalesce($9::date, valid_until)
       WHERE company_id=$1 AND id=$2`,
      [companyId, req.params.id, subtotal, tax, subtotal + tax, taxRate, body.notes ?? null, body.description ?? null, body.validUntil || null],
    );
  }
...
estimatesRouter.delete('/:id', requirePermission('estimates.write'), tenantRoute(async (req, res, client) => {
  ...
  if (!r.rowCount) throw notFound('Estimate');
  res.json({ ok: true });
}));
...
estimatesRouter.post('/:id/status', requirePermission('estimates.write'), tenantRoute(async (req, res, client) => {
  ...
  const r = await client.query(
    `UPDATE estimates SET status = $3 WHERE company_id = $1 AND id = $2 RETURNING id, estimate_number`,
    [req.auth.companyId, req.params.id, body.status],
  );
  if (!r.rowCount) throw notFound('Estimate');
  if (body.status === 'sent') {
    const est = await mapEstimate(client, req.auth.companyId!, req.params.id);
...
estimatesRouter.post('/:id/convert', requirePermission('estimates.write'), tenantRoute(async (req, res, client) => {
  ...
  if (est.status !== 'approved') throw badRequest('Estimate must be approved before conversion');
  ...
  await client.query(
    `INSERT INTO job_line_items (company_id, job_id, description, quantity, unit_price, total, taxable)
     SELECT company_id, $2, description, quantity, unit_price, total, true FROM estimate_line_items WHERE estimate_id = $1`,
    [req.params.id, jobId],
  );
  await client.query(
    `INSERT INTO job_assignees (company_id, job_id, member_id)
     SELECT company_id, $2, member_id FROM estimate_assignees WHERE estimate_id = $1`,
    [req.params.id, jobId],
  );
  await client.query(
    `INSERT INTO job_notes (company_id, job_id, author_id, body) VALUES ($1,$2,$3,$4)`,
    [companyId, jobId, req.auth.userId, `Created from estimate ${est.estimateNumber}`],
  );
  await client.query(
    `UPDATE estimates SET status = 'converted', converted_job_id = $2 WHERE id = $1`,
    [req.params.id, jobId],
  );
  await audit(client, {
